    ...
    >
      {data.map((item, key) => (
        ...
        >
          ...
          {hasSeparator && hasDescription && <hr className="mt-10" />}
          {item.description && hasDescription && (
            <div className="max-w570 mt-10 description">{item.description}</div>
          )}
          
          {item.href && (
            <Button className={"link-custom mt-10"} {...item.href}>
              {item.buttonText}
            </Button>
          )}
        </div>
